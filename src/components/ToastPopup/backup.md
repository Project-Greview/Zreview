...
const ToastPopup: React.FC<ToastPopupProps> = ({ ready }) => {
  ...
  return (
    <div
      className={`toast_sections absolute ${
        toastModal && loading ? "active" : ""
      }`}
    >
      <Swiper
        className="toast_controll_section"
        direction={"vertical"}
        centeredSlides={true}
        slidesPerView={2}
      >
        <SwiperSlide className="toast_bg"></SwiperSlide>
        <SwiperSlide className="toast_view">
          <div>
            <div className="toast_header flex flex_dir_c flex_jc_c flex_ai_c">
              <div className="drag_icon"></div>
              {searchType ? (
                  ...
                  {locationResult.result.map((item: any, index: number) => {
                    ...
                    return (
                      <li
                        key={item.id}
                        ref={
                          index > locationResult.result.length - 1 ? ref : null
                        }
                      >
                        <ResultItem data={item} range={distance} />
                      </li>
